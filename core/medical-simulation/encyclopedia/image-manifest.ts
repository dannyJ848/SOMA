...
import { indexedImages, getImageCount, type IndexedImage } from '../../content/generated-image-index';
...
/**
 * Convert an IndexedImage (from generated index) to a MedicalImage
 * with basic auto-generated metadata.
 */
function indexedToMedicalImage(img: IndexedImage): MedicalImage {
  ...
  const categoryMap: Record<string, MedicalImage['category']> = {
  ...
  return {
    ...
    title,
    path: img.path,
    format: img.format as MedicalImage['format'],
    category: categoryMap[img.category] || 'diagram',
    systems: img.system ? [img.system] : [],
    description: `${img.category} image: ${title}`,
    attribution: {
      source: 'Medical Image Library',
      license: 'Educational Use',
    },
    level: 'intermediate',
    tags: [img.category, img.system, img.filename.replace(/\.[^.]+$/, '')].filter(Boolean) as string[],
    relatedEntryIds: [],
  };
}

/**
 * Get all medical images: hardcoded manifest entries merged with
 * discovered images from the generated index.
 *
 * Hardcoded entries (with rich metadata) take priority over auto-generated ones.
 */
export function getAllMedicalImages(): MedicalImage[] {
  const hardcodedIds = new Set(allMedicalImages.map(img => img.id));

  // Convert discovered images, excluding any that match hardcoded IDs
  const discovered = indexedImages
    .filter(img => !hardcodedIds.has(img.id))
    .map(indexedToMedicalImage);

  return [...allMedicalImages, ...discovered];
}

/**
 * Get total count of all available images (hardcoded + discovered)
 */
export function getTotalImageCount(): number {
  return allMedicalImages.length + getImageCount();
}
