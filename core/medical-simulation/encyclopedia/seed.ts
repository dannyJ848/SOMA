  ...
  for (const [regionId, region] of Object.entries(REGION_CONTENT_MAP)) {
    ...
    for (const condition of region.pathology.commonConditions) {
      ...
      entries.push({
        ...
        references: buildDefaultReferences(),
        searchMetadata: {
          primaryKeywords: [condition.name.toLowerCase(), ...condition.symptoms.map(s => s.toLowerCase())],
    ...
    for (const tissue of region.histology.tissueTypes) {
      ...
      entries.push({
        ...
        references: buildDefaultReferences(),
        searchMetadata: {
          primaryKeywords: [tissue.name.toLowerCase(), tissue.category],
    ...
    for (const process of region.physiology.processes) {
      ...
      entries.push({
        ...
        references: buildDefaultReferences(),
        searchMetadata: {
          primaryKeywords: [process.name.toLowerCase()],
    ...
    for (const func of region.physiology.functions) {
      ...
      entries.push({
        ...
        references: buildDefaultReferences(),
        searchMetadata: {
          primaryKeywords: [func.name.toLowerCase()],
