...
export function addEntry(entry: EncyclopediaEntry): void {
  ...
  // Index Spanish name
  if (entry.nameEs) {
    const normalizedNameEs = entry.nameEs.toLowerCase();
    if (!keywordIndex.has(normalizedNameEs)) {
      keywordIndex.set(normalizedNameEs, new Set());
    }
    keywordIndex.get(normalizedNameEs)!.add(entry.entryId);
  }
...
export function searchEntries(
  ...
  for (const entry of encyclopediaDatabase.values()) {
    ...
    const lowerNameEs = entry.nameEs?.toLowerCase() ?? '';

    // Check exact name match (English or Spanish — ranked equally)
    if (lowerName === normalizedQuery || (lowerNameEs && lowerNameEs === normalizedQuery)) {
      matchType = 'exact';
      relevanceScore = 100;
    }
    // Check synonym match
    ...
    // Check primary keyword match (includes Spanish keywords added via seed.ts)
    else if (
      entry.searchMetadata.primaryKeywords.some((k) => {
        const lk = k.toLowerCase();
        return lk.includes(normalizedQuery) || normalizedQuery.includes(lk);
      })
    ) {
      matchType = 'keyword';
      relevanceScore = 70;
    ...
    // Check name contains query or query contains name (English or Spanish — ranked equally)
    else if (
      lowerName.includes(normalizedQuery) || normalizedQuery.includes(lowerName) ||
      (lowerNameEs && (lowerNameEs.includes(normalizedQuery) || normalizedQuery.includes(lowerNameEs)))
    ) {
      matchType = 'partial';
      // Score partial matches by overlap ratio so "Cardiology" ranks higher than "Card"
      const bestOverlap = Math.max(
        lowerName.includes(normalizedQuery) || normalizedQuery.includes(lowerName)
          ? Math.min(normalizedQuery.length, lowerName.length) / Math.max(normalizedQuery.length, lowerName.length)
          : 0,
        (lowerNameEs && (lowerNameEs.includes(normalizedQuery) || normalizedQuery.includes(lowerNameEs)))
          ? Math.min(normalizedQuery.length, lowerNameEs.length) / Math.max(normalizedQuery.length, lowerNameEs.length)
          : 0
      );
      relevanceScore = 40 + bestOverlap * 15; // 40–55 range based on overlap quality
    }
 ...
 * Checks English name, Spanish name, aliases, and synonyms.
...
export function getEntryByName(name: string): EncyclopediaEntry | undefined {
  ...
  for (const entry of encyclopediaDatabase.values()) {
    ...
    if (entry.nameEs && entry.nameEs.toLowerCase() === normalizedName) {
      return entry;
    }
  ...
  // --- Pass 2: partial matches (query contained in name, or name contained in query) ---
  let bestPartial: EncyclopediaEntry | undefined;
  let bestPartialScore = 0;

  for (const entry of encyclopediaDatabase.values()) {
    const candidates: string[] = [entry.name];
    if (entry.nameEs) candidates.push(entry.nameEs);
    if (entry.aliases) candidates.push(...entry.aliases);
    if (entry.searchMetadata.synonyms) candidates.push(...entry.searchMetadata.synonyms);

    for (const candidate of candidates) {
      const lowerCandidate = candidate.toLowerCase();
      if (lowerCandidate.includes(normalizedName) || normalizedName.includes(lowerCandidate)) {
        // Score: ratio of query length to candidate length (closer to 1 = better match)
        const score = Math.min(normalizedName.length, lowerCandidate.length) /
                      Math.max(normalizedName.length, lowerCandidate.length);
...
/**
 * Get an entry by its Spanish name.
 * Tries exact match first, then partial matches (query in name or name in query).
 */
export function getEntryBySpanishName(nameEs: string): EncyclopediaEntry | null {
  const lowerName = nameEs.toLowerCase().trim();
  if (!lowerName) return null;

  // Pass 1: exact case-insensitive match on nameEs
  for (const entry of encyclopediaDatabase.values()) {
    if (entry.nameEs && entry.nameEs.toLowerCase() === lowerName) {
      return entry;
    }
  }

  // Pass 2: partial match — query contained in nameEs, or nameEs contained in query
  let bestPartial: EncyclopediaEntry | null = null;
  let bestScore = 0;

  for (const entry of encyclopediaDatabase.values()) {
    const entryNameEs = entry.nameEs?.toLowerCase();
    if (!entryNameEs) continue;

    if (entryNameEs.includes(lowerName) || lowerName.includes(entryNameEs)) {
      const score = Math.min(lowerName.length, entryNameEs.length) /
                    Math.max(lowerName.length, entryNameEs.length);
      if (score > bestScore) {
        bestScore = score;
        bestPartial = entry;
      }
