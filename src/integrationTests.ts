...
export async function runIntegrationTests(): Promise<{
  ...
  // Test 2: Get stats
  await test('Get content stats', () => {
    const stats = getUniversalContentStats();
    return stats.totalContent > 0;
  });

  // Test 3: Search brain content
  await test('Search brain content', () => {
    const results = searchUniversalContent('brain');
    return results.length > 0;
  });

  // Test 4: Search heart content
  await test('Search heart content', () => {
    const results = searchUniversalContent('heart');
    return results.length > 0;
  });

  // Test 5: Get by structure - cerebral cortex
  await test('Get cerebral cortex content', () => {
    const content = getContentByStructure('cerebral-cortex');
    return content !== null;
  });

  // Test 6: Get by structure - heart
  await test('Get heart content', () => {
    const content = getContentByStructure('heart');
    return content !== null;
  });

  // Test 7: Get by structure - lungs
  await test('Get lungs content', () => {
    const content = getContentByStructure('lungs');
    return content !== null;
  });

  // Test 8: Get by structure - liver
  await test('Get liver content', () => {
    const content = getContentByStructure('liver');
    return content !== null;
  });

  // Test 9: Get by structure - kidneys
  await test('Get kidneys content', () => {
    const content = getContentByStructure('kidneys');
    return content !== null;
  });

  // Test 10: Search pathology
  await test('Search pathology content', () => {
    const results = searchUniversalContent('stroke');
    return results.length > 0;
  });
